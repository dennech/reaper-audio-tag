...
- Если тормоза остаются, сохрани текущий log и отметь, просадка начинается на `Preparing audio...` или уже на `Listening...`.
- Приложи скриншот блока `Diagnostics` прямо из окна отчёта.
- Нажми `Debug log` в окне отчёта и пришли этот telemetry log вместе с обычным runtime/export log.
