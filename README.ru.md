...
5. Показывает:
   ...
   - до `5` top cues и `6` top tags в compact view
...
- В режиме `More` есть selector `Icons: Auto / Emoji / Symbols`. `Auto` использует системный font fallback ReaImGui для mixed emoji text, а `Symbols` принудительно включает безопасный symbol fallback.
- Для диагностики export без запуска модели используй `reaper/PANNs Item Report - Debug Export.lua`.
