...
Проект использует AudioSet tagging модель PANNs `Cnn14` от Qiuqiang Kong, Yin Cao, Turab Iqbal, Yuxuan Wang, Wenwu Wang и Mark D. Plumbley. Release-модель — это ONNX-экспорт upstream checkpoint `Cnn14_mAP=0.431.pth`, подготовленный для локальной работы в REAPER.

Подробности и maintainer-only workflow для конвертации описаны в [docs/model.ru.md](docs/model.ru.md).
